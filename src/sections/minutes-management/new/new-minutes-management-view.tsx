...
export default function NewMinutesManagementView({ id }: MinutesFormViewProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error(isEdit ? 'Failed to update instructor' : 'Failed to create instructor');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Box sx={{ py: 2, pb: 6 }}>
        ...
        <Box>
          ...
          >
            ...
            <Typography sx={{ fontSize: 13, color: '#1E293B', fontWeight: 600 }}>
              {isEdit ? t('breadcrumb_edit_lecturer') : t('breadcrumb_add_lecturer')}
            </Typography>
          </Breadcrumbs>
      ...
      <Stack direction="row" spacing={2} sx={{ justifyContent: 'flex-start' }}>
        ...
        >
          {loading ? <CircularProgress size={18} color="inherit" /> : <Iconify icon="mingcute:check-line" width={20} />}
