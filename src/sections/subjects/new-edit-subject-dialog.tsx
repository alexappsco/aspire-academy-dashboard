...
import { MOCK_COLLEGES } from 'src/sections/college/_mock';

import { SubjectItem } from './_mock';

interface SubjectFormDialogProps {
  open: boolean;
  onClose: () => void;
  initialData?: SubjectItem | null;
  onSave: (data: Partial<SubjectItem>) => void;
}
...
export default function SubjectFormDialog({
  ...
  const filteredColleges = useMemo(
    () => MOCK_COLLEGES.filter((c) => c.universityId === universityId && c.active),
    [universityId]
  );
  ...
  return (
    ...
    >
      ...
      <DialogContent sx={{ pt: 0, px: 3, pb: 3 }}>
        ...
        {/* College Select (filtered by university) */}
        <SelectField
          fullWidth
          size="small"
        ...
        >
          <MenuItem value="">
            <em>{t('dialog.college_placeholder')}</em>
          </MenuItem>
