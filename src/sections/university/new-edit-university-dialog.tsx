...
import { UniversityItem } from './_mock';

interface UniversityFormDialogProps {
  open: boolean;
  onClose: () => void;
  initialData?: UniversityItem | null;
  onSave: (data: Partial<UniversityItem>) => void;
}
...
export default function UniversityFormDialog({
  ...
  const [countryAr, setCountryAr] = useState(initialData?.country_ar ?? '');
  const [countryEn, setCountryEn] = useState(initialData?.country_en ?? '');
  ...
  const handleSubmit = () => {
    onSave({
      ...
      order,
      active,
      logo: previewUrl || '/icons/package.svg',
    });
    onClose();
  };
      ...
      <DialogContent sx={{ pt: 0, px: 3, pb: 3 }}>
        ...
        <Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
          <TextField
            fullWidth
            size="small"
            label={t('dialog.country_ar')}
            placeholder={t('dialog.country_ar_placeholder')}
            value={countryAr}
          ...
          <TextField
            fullWidth
            size="small"
            label={t('dialog.country_en')}
            placeholder={t('dialog.country_en_placeholder')}
            value={countryEn}
